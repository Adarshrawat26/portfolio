...
function Chip({ children, icon }: { children: string; icon?: string | null }) {
  return (
    <span className="inline-flex items-center gap-1.5 text-xs px-3 py-1 rounded-md border font-medium bg-paper-2 text-ink-2 border-rule">
    ...
    >
      ...
      <div className="rounded-[var(--radius-card-quiet)] border border-rule overflow-hidden bg-paper-2">
        <div className="flex items-center gap-2 px-3 py-2 border-b border-rule bg-paper-2">
          ...
          <div className="flex-1 bg-paper border border-rule rounded-md px-2.5 py-0.5 text-3xs text-ink-3 font-medium truncate">
            {hostname}
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start gap-6 sm:gap-8">
        <div className="flex flex-col min-w-0 flex-1 sm:order-1">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            ...
            <span className="text-3xs font-semibold uppercase tracking-[0.12em] text-[var(--color-accent-3-deep)] bg-paper border border-[var(--color-accent-3)] rounded-full px-2 py-0.5">
              {item.status}
            </span>
          </div>

          <p className="text-lg sm:text-xl font-bold text-ink leading-snug mb-2">{item.name}</p>
          <p className="text-sm sm:text-base text-ink-2 leading-relaxed font-normal mb-4">{item.tagline}</p>

          <ul className="space-y-1.5 mb-5">
            {item.highlights.map((h) => (
              <li key={h} className="text-xs sm:text-sm text-ink-2 leading-relaxed font-normal flex gap-2">
